...
export default function Home() {
    ...
    const onClickDownloadRecords = async (event: React.MouseEventHandler<HTMLButtonElement>) => {
        ...
        if (tables) {
            ...
            tableArray.forEach(async (table: any) => {
                let url = `http://localhost:6969/query/${database}&table=${table}&select=*&where=1=1&apikey=${connectionKey}`
    ...
    const handleDatabaseNameSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        let url = `http://localhost:6969/getkey/${database}&apikey=${connectionKey}`;
    ...
    const syncSchemaandRecords = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (tables) {
            const tableArray = JSON.parse(tables);
            tableArray.forEach(async (table: any) => {
                let url = `http://localhost:6969/query/${database}&table=${table}&select=*&where=1=1&expand=true&apikey=${connectionKey}`
                const response = await fetch(url, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            <div className="z-10 w-full max-w-5xl items-center justify-between font-mono text-sm lg:flex">
                <div className="fixed bottom-0 left-0 flex h-48 w-full items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black lg:static lg:h-auto lg:w-auto lg:bg-none">
                    <div className="flex flex-col items-center justify-center gap-8">
                        ...
                        <Link href="/viewupdate" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
